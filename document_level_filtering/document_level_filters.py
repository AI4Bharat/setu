...
def __get_lang_code(lang: str):
    # TODO: Read json from iso file and return value
    return "en"


def document_level_filters(data: Dict[str, str], ngram_start: int, ngram_end: int):
    lid_res = (("English", {}), {})  # TODO: Call LID and get response
    lang, _ = lid_res[0]
    lang_code = __get_lang_code(lang)

    output = {
        "identifier": 0,
        "source": data["source"],
        "text": sentence_split(data["body"], lang_code),
        "word_distribution": calc_word_count(data["body"], lang_code),
        ...
        "nsfw_word_distribution": calc_nsfw_word_distribution(data["body"], lang_code),
