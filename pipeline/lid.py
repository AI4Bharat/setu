...
class LIDPipeline():
    ...
    def run_single(self, text, for_spark=True):
...
class LIDSparkPipeline(LIDPipeline):

    def __init__(self, data_folder, input_threshold=0.5, romain_lid_threshold=0.6):

        IndicLID_FTN_path = os.path.join(data_folder, 'models', 'indiclid-ftn', 'model_baseline_roman.bin')
        IndicLID_FTR_path = os.path.join(data_folder, 'models', 'indiclid-ftr', 'model_baseline_roman.bin')
        IndicLID_BERT_path = os.path.join(data_folder, 'models', 'indiclid-bert', 'basline_nn_simple.pt')
        nllb_model_path = os.path.join(data_folder, 'models', 'lid218e.bin')
        mapping_json_path = os.path.join(data_folder, 'language_mapping.json')
        ...
        super().__init__(
            ...
            roman_lid_threshold=romain_lid_threshold, 
            nllb_model_path=nllb_model_path,
            mapping_json_path=mapping_json_path,
        )

        self.run_single_udf = udf(self.run_single, StringType())

    def run_spark_pipeline(self, df, text_column):

        df = df \
            .withColumn("detected_lang", self.run_single_udf(text_column))
